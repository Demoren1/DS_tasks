...
import ale_py.env as atari_env
import numpy as np
...
class MaxBetweenFrames(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        if (isinstance(env.unwrapped, atari_env.AtariEnv) and
                env.spec.kwargs['frameskip'] != 1):
            raise ValueError(
...
def nature_dqn_env(env_id, kwargs, nenvs=None, seed=None,
    ...
    if nenvs is not None:
        ...
        env = ParallelEnvBatch([
            lambda i=i, env_seed=env_seed: nature_dqn_env(
                env_id, kwargs, seed=env_seed, summaries=False, clip_reward=False)
