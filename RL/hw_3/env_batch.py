...
class EnvBatch(Env):
    ...
    def step(self, actions):
        ...
        obs, rews, resets, truncates, infos = [], [], [], [], []
        ...
        return np.stack(obs), np.stack(rews), np.stack(resets), np.stack(truncates), infos
...
class SingleEnvBatch(Wrapper, EnvBatch):
    ...
    def step(self, actions):
        self._check_actions(actions)
        ob, rew, done, truncated, info = self.env.step(actions[0])
        if done:
            ob, _ = self.env.reset()
        return (
            ob[None],
...
def worker(parent_connection, worker_connection, make_env_function,
    ...
    while True:
        cmd, action, seed, options = worker_connection.recv()
        if cmd == "step":
            ob, rew, done, truncated, info = env.step(action)
            if done:
                ob, _ = env.reset()
            worker_connection.send((ob, rew, done, truncated, info))
        elif cmd == "reset":
            ob, info = env.reset(seed=seed, options=options)
            worker_connection.send((ob, info))
        elif cmd == "close":
            env.close()
...
class ParallelEnvBatch(EnvBatch):
    ...
    def step(self, actions):
        ...
        for conn, a in zip(self._parent_connections, actions):
            conn.send(("step", a, None, None))
        results = [conn.recv() for conn in self._parent_connections]
        obs, rews, dones, truncates, infos = zip(*results)
        return np.stack(obs), np.stack(rews), np.stack(dones), np.stack(truncates), infos

    def reset(self, seed=None, options=None):
        for conn in self._parent_connections:
            conn.send(("reset", None, seed, options))
