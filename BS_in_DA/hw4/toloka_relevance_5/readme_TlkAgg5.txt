This file describes the data set TlkAgg5 available at https://toloka.ai/datasets.

The data set is intended for non-commercial use. You must indicate that the data was obtained using Toloka. If you plan to use the datasets for commercial purposes, obtain consent from Toloka by contacting: toloka@support.yandex.com. 

The task was to assess the relevance of a document for a search query on a 5-graded scale. Some tasks in this data set have more than one ground truth (golden) label, in such cases, all the golden labels are considered equally correct. The main metric of quality is the accuracy of aggregated labels estimated as the fraction of the aggregated labels equal to one of the golden labels for a given task from the golden set.
